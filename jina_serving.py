...
class KnowledgeBasedChatLLM:
    ...
    def get_knowledge_based_answer(self,
        ...
        if web_content:
            ...
            prompt_template = """您现在已经成为一名成熟的司机。下面将在问题中告诉你详细的道路上车辆的情况，每次提问之后根据已知内容选择一个动作。
                注意：你只能选择一个动作,不用解释!不用解释！
...
if __name__ == "__main__":
    ...
    vector_store(file_path='./action.txt')

    predict('您在一条有5条车道的道路上行驶，并且您目前在最右侧的车道上行驶。您当前的位置为（322.81，16.00），速度为25.00 m/s，加速度为0.00 m/s ^2，车道位置为322.81 m。您周围还有其他车辆在行驶，以下是它们的基本信息：车辆464与您在同一车道上行驶，在您前面。它的位置是（350.29，16.00），速度是25.00 m/s，加速度是0.00 m/s ^2，车道位置是350.29 m。车辆208在你左边的车道上行驶，在你前面。其位置为（383.35，12.00），速度为25.00 m/s，加速度为0.00 m/s ^2，车道位置为383.35 m。您选择哪一个动作呢？',
            use_web=False,
            top_k=6,
            history_len=3,
            temperature=0.01,
